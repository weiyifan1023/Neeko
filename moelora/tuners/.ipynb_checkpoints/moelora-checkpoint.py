...
class MoeLoraConfig(PeftConfig):
    ...
    top_n_for_moe: int = field(default=2, metadata={"help": "Select the top n for each MoeLora"})
    gating: str = field(default="Standard", metadata={"help": "Select the gating network for each MoeLora"})
    loss_fn: str = field(default="None", metadata={"help": "Loss function"})
    L_factor: float = field(default=0.01, metadata={"help": "Factor of Load Balancing Loss"})
    Z_factor: float = field(default=0.01, metadata={"help": "Factor of Z-Loss"})
    O_factor: float = field(default=0.01, metadata={"help": "Factor of O-Loss"})
    V_factor: float = field(default=0.01, metadata={"help": "Factor of V-Loss"})
    var_lower_bound: float = field(default=0.5, metadata={"help": "Lower bound for V-Loss"})
    ...
    **Attributes**:
    ...
    def _create_new_module(self, moelora_config, adapter_name, target):
        ...
        kwargs = {
            ...
            "loss_fn": moelora_config.loss_fn,
            "L_factor": moelora_config.L_factor,
            "Z_factor": moelora_config.Z_factor,
            "O_factor": moelora_config.O_factor,
            "V_factor": moelora_config.V_factor,
            "var_lower_bound": moelora_config.var_lower_bound,
            "top_n": moelora_config.top_n_for_moe,
        ...
        for key in key_list:
            ...
            if isinstance(target, MoeLoraLayer) and isinstance(target, torch.nn.Conv2d):
                ...
                target.update_layer(
                    ...
                    moelora_config.top_n_for_moe,
                    moelora_config.gating,
                    moelora_config.loss_fn,
                    moelora_config.L_factor,
                    moelora_config.Z_factor,
                    moelora_config.O_factor,
                    moelora_config.V_factor,
                    moelora_config.var_lower_bound,
                    moelora_config.lora_alpha,
                    moelora_config.lora_dropout,
                    moelora_config.init_lora_weights,
                )
...
class InfoNCE:
    def __init__(self, q_len_per_expert=128, num_moe=8, temperature=0.07):
        self.q_len_per_expert = q_len_per_expert
        self.num_moe = num_moe
        self.temperature = temperature
        self.queue = [torch.empty(0).cuda() for _ in range(num_moe)]
    
    def update_queue(self, selected_tokens, experts_capacity):
        tokens_split = torch.split(selected_tokens, experts_capacity.tolist(), dim=0)
        for i in range(self.num_moe):
            if experts_capacity[i] == 0:
                continue
            self.queue[i] = torch.cat([self.queue[i], tokens_split[i]], dim=0)
            if len(self.queue[i]) > self.q_len_per_expert:
                self.queue[i] = self.queue[i][-self.q_len_per_expert:]
    
    def sampler(self):
        experts_capacity = [len(q) for q in self.queue]
        queue = torch.cat(self.queue, dim=0)
        return queue, experts_capacity
    
    def mask_maker(self, experts_capacity, queue_experts_capacity):
        experts_position = torch.cat([torch.full((capacity,), i) for i, capacity in enumerate(experts_capacity)])
        queue_position = torch.cat([torch.full((capacity,), i) for i, capacity in enumerate(queue_experts_capacity)])
        mask = (experts_position.unsqueeze(1) == queue_position.unsqueeze(0)).bool()
        return mask.to(experts_capacity.device)

    def loss_calculate(self, A_out, Gate):
        batch_size, seq_len, n, dim = A_out.shape
        A_out = A_out / A_out.norm(dim=-1, keepdim=True)
        A_out = A_out.permute(2, 0, 1, 3).reshape(n * batch_size * seq_len, dim)
        experts_select = (Gate > 1e-4).bool()
        experts_select = experts_select.permute(2, 0, 1, 3).reshape(n, batch_size * seq_len)
        experts_capacity = experts_select.sum(dim=-1)
        experts_select = experts_select.reshape(-1, 1)
        selected_tokens = torch.masked_select(A_out, experts_select).reshape(-1, dim)
        self.update_queue(selected_tokens.clone(), experts_capacity)
        queue, queue_experts_capacity = self.sampler()
        product = torch.matmul(selected_tokens, queue.detach().transpose(0, 1))
        product = torch.exp(product / self.temperature)
        mask = self.mask_maker(experts_capacity, queue_experts_capacity)
        loss = - torch.log(product / product.sum(dim=-1, keepdim=True)) * mask
        count = torch.nonzero(loss).size(0)
...
#         self.num_moe = num_moe
#         self.temperature = temperature
#         self.queue = torch.empty(0).cuda()
#         self.queue_select = torch.empty(0).cuda()
    
#     def update_queue(self, A_out, experts_select):
#         self.queue = torch.cat([self.queue, A_out], dim=1)
#         self.queue_select = torch.cat([self.queue_select, experts_select], dim=1)
#         n, q_len, dim = self.queue.size()
#         if q_len > self.q_len_per_expert:
#             self.queue = self.queue[-self.q_len_per_expert:]
#             self.queue_select = self.queue_select[-self.q_len_per_expert:]
...
#         experts_select = (Gate > 1e-4).bool()
#         experts_select = experts_select.permute(2, 0, 1, 3).reshape(n, batch_size * seq_len)
#         self.update_queue(A_out.clone(), experts_select)
#         p_mask = experts_select.unsqueeze(2) * self.queue_select.unsqueeze(1)
#         product = torch.matmul(A_out, self.queue.transpose(1, 2))
#         product = torch.exp(product / self.temperature)
#         loss = - torch.log(product / product.sum()) * p_mask
#         count = torch.nonzero(loss).size(0)
#         return loss.sum() / count


def NCELoss(A_out, Gate):
    temperature = 0.07
    batch_size, seq_len, n, dim = A_out.shape
    A_out = A_out / A_out.norm(dim=-1, keepdim=True)
    A_out = A_out.permute(2, 0, 1, 3).reshape(n * batch_size * seq_len, dim)
    ...
    selected_tokens = torch.masked_select(A_out, experts_select).reshape(-1, dim)
    experts_position = torch.cat([torch.full((capacity,), i) for i, capacity in enumerate(experts_capacity)])
    mask = (experts_position.unsqueeze(1) == experts_position.unsqueeze(0)).bool().to(A_out.device)
    product = torch.matmul(selected_tokens, selected_tokens.transpose(0, 1))
    product = torch.exp(product / temperature)
    loss = - torch.log(product / product.sum()) * mask
    count = torch.nonzero(loss).size(0)
    return loss.sum() / count
...
#     product = torch.exp(product / temperature)
#     p_product = product
#     loss = - torch.log(p_product / product.sum(dim=-1, keepdim=True)) * p_mask
#     return loss.mean()
...
class MoeLoraLayer:
    ...
    def update_layer(self, adapter_name, r, num_moe, top_n_for_moe, gating, loss_fn, L_factor, Z_factor, V_factor, var_lower_bound, lora_alpha, lora_dropout,
                     init_lora_weights):
        self.r[adapter_name] = r
        self.num_moe[adapter_name] = num_moe
        self.lora_alpha[adapter_name] = lora_alpha
        self.gating.update(
            nn.ModuleDict({adapter_name: GATING_TO_MODEL_MAPPING[gating](self.in_features, num_moe, top_n_for_moe, loss_fn, L_factor, Z_factor, V_factor, var_lower_bound)}))
        if lora_dropout > 0.0:
...
class Linear(nn.Linear, MoeLoraLayer):
    ...
    def __init__(
        ...
        self.infonce = InfoNCE(q_len_per_expert=128, num_moe=num_moe)
        self.O_factor = O_factor
        self.fan_in_fan_out = fan_in_fan_out
        if fan_in_fan_out:
            self.weight.data = self.weight.data.T
        nn.Linear.reset_parameters(self)
        self.update_layer(adapter_name, r, num_moe, top_n, gating, loss_fn, L_factor, Z_factor, V_factor, var_lower_bound, lora_alpha, lora_dropout, init_lora_weights)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            Gate = self.gating[self.active_adapter](x).unsqueeze(-1)
            Gate_out = A_out * Gate
            result += (
                self.lora_B[self.active_adapter](
                    Gate_out.reshape(batch_size, seq_len, -1)
                    )
                    * self.scaling[self.active_adapter]
                )
            if self.training and self.loss_fn != "None" and "O" in self.loss_fn:
                # OLoss = self.O_factor * expert_loss(A_out, Gate)
                # OLoss = self.O_factor * NCELoss(A_out, Gate)
                OLoss = self.O_factor * self.infonce.loss_calculate(A_out, Gate)
                # self.cnt += 1
                # if self.cnt % 50 == 0:
                #     print(f"NCE Loss:{OLoss}")
                OLoss.requires_grad_(True)
                OLoss.backward(retain_graph=True)
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
...
if is_bnb_available():
    ...
    class Linear8bitLt(bnb.nn.Linear8bitLt, MoeLoraLayer):
        ...
        def __init__(
            ...
            self.weight.requires_grad = False
            init_lora_weights = kwargs.pop("init_lora_weights", True)
            self.update_layer(adapter_name, r, num_moe, top_n, gating, loss_fn, lora_alpha, lora_dropout, init_lora_weights)
            self.active_adapter = adapter_name
