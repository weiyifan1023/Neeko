...
for k in ROLE_PROFILE_MAPPING.keys():
    ROLE_PROFILE_MAPPING[k] = torch.load(os.path.join("/home/tongxuluo/Neeko/role_embd", k + ".pth")).unsqueeze(0).cuda()
...
def main(args):
    ...
    if hasattr(model, "global_role_embd"):
        model.global_role_embd.append(ROLE_PROFILE_MAPPING[args.character])
