...
def parse_arguments():
    ...
    parser.add_argument(
        "--save_path", type=str,
        default='../data/gen_results/rag_interview_single/{name}_{baseline}_result/{name}_single.json'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--baseline", type=str, default="chatgpt"
    )
    parser.add_argument(
...
def gpt4_evaluator(inputs, character, rag):
...
def main(args):
    ...
    for i, one in enumerate(tqdm(test_set)):
        ...
        _, profile = read_profile(os.path.join(seed_data_dir, 'profiles', f'wiki_{args.character}.txt'))
        # llms
        # res = evaluate(tokenizer=tokenizer, model=model, character=args.character, inputs=inputs, rag=profile[0])
        # chatgpt
        res = gpt4_evaluator(character=args.character, inputs=inputs, rag=profile[0])
        reply = {
            "role": args.character,
            "action": "(speaking)",
            "content": res,
