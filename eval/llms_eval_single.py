...
parser.add_argument("--aspect",
                    choices=['behavior', 'utterance', 'relevance', 'stability', 'hallucinatory', 'real', 'virtual',
                             'transfer'],
                    default="behavior",
...
parser.add_argument("--model", default="Neeko-7b", type=str)  # 选择 base LLM
# select base LLM
...
gen_results_dir = f'../data/gen_results/Neeko_expansion_interview_{args.mode}'
output_dir = '../data/score_results/Neeko_expansion_interview_{mode}/{model_name}'
# output_dir = '../data/score_results/Neeko_expansion_interview_{mode}/{character_name}_{model_name}'
data_dir = '../data/'
aspect = args.aspect
print("Aspect: ", aspect)
assert aspect in ['behavior', 'utterance', 'relevance', 'stability',
                  'transfer', 'hallucinatory', 'real', 'virtual'], f'aspect:{aspect} is not supported!'
...
def eval_interview(prompt_ds):
    for ex in prompt_ds[args.start:]:
        ...
        qid = ex["qid"]
        # question = ex["question"]
        save_path = output_dir.format(mode=args.mode, character_name=character_name, model_name=model_name)
